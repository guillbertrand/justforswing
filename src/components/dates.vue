        ...
        <div class="mb-10 font-open">
          <div v-for="gig in gigs.slice(-10)" :key="gig.ID" > 
                <div v-if="displayGig(gig)" class="flex flex-col mb-4 text-white justify-center">
                  <span class="font-bold text-xl lg:text-3xl text-yellowJfs x">{{ gig.DATE | momentDate  }}</span>
                  <span>{{ displayGigTitle(gig)}}</span>
                  <span class="text-sm lg:text-xl font-bold" v-if="displayGigTitle(gig) == 'Concert public'">
